...
```{r}
my_missing <- NA      #the object my_missing gets NA
```

```{r}
is.na(my_missing)     # is NA in my_missing?
```

```{r}
anyNA(my_missing)     # any NAs in my_missing?
```
...
The NA disables you from performing arithmetic on data. 
