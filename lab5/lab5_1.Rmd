...
```
```{r}
glimpse(mammals) #Note how some character data would be better represented as Factors 
mammals %>% 
...
```{r}
fish_sub <- fish %>%
  filter(lakeid == c("AL", "AR")) %>%
  filter(between(radii_length_mm, 2,4)) %>% 
  arrange(desc(fish_id))
```
...
Make a plot of max_life and litter_size 


```{r}
ggplot(mammals_new, aes(x=litter_size, y=max_life_new))+
  geom_point()+
  geom_smooth()+
  labs(title = "Mammal Life Spans by Litter Sizes", 
       x = "Litter Size", 
       y = "Max Life (years)")
