...
The data remains as a numeric or string, it doesn't get converted to a string like it does in a matrix 
...
```{r}
hspring$depth_ft <- c(4.15, 4.13, 4.12, 3.21, 3.23, 3.20, 5.67, 5.65, 5.66)
hspring
```

4. Calculate the mean temperature of all of the temperature measurements.
```{r}
temps <- hspring$temp_c
mean(temps)
```

5. Save your hot springs data as a `.csv` file! Do not allow row names. 
```{r}
write.csv(hspring, "hot_spring_data.csv", row.names = FALSE)
```
