...
The scientist column is a Character class 
```{r}
...
fish <- read_csv("data/Gaeta_etal_CLC_data.csv.csv") 
...
```
lakeid and annnumber are character type,the rest are numerical. 
...
###Create Data Visual Using ggplot2
```{r}

ggplot(mammals, aes(x=order, y=mass), geom_point(na.rm=T), geom_line() + 
         theme(axis.title.x = ))
