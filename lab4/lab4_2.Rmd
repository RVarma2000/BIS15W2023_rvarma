...
```{r}
filter(fish, lakeid == "AL" & length > 350) #equivalent to and
```

Notice that the `|` operator generates a different result.
```{r}
filter(fish, lakeid == "AL" | length > 350)  #equivalent to and/or 
```
...
```{r}
mammals2 <- select(mammals, `family` , `genus` , `species` , `mass` , `gestation` )
```

5. Use the output from #4 to focus on the family Bovidae with a mass greater than 450000.
```{r}
bigbovidae <- filter(mammals2, family == "Bovidae" & mass > 450000 )
```
...
```{r}
mammals3 <- select(mammals, genus|species|mass|gestation|newborn)

mammals4 <- filter(mammals3, genus %in% c("Lophocebus", "Eryhtrocebus", "Macaca"))
```

```{r}
smallest_newborn <- filter(mammals4, newborn == min(newborn))
smallest_newborn
```
The Macaca maura has the smallest newborn mass. 
## Wrap-up  
