...
names(mammals)
max_life <- mammals[,11]
max_life_avg <- mean(max_life)
```
Yes there are many NAs as shown by summary(mammals) and the fact that trying to calculate the mean of a numerical object returned NA. 
...
```{r}
mammals <- rename(mammals,genus = Genus,afr = AFR,max_life = `max. life`,litters_yr = `litters/year`)
