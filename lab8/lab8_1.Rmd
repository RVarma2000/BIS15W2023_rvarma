...
```{r}
here()   #useful when you have multiple data set folders and you don't want to change working directories each time you load data 
```
...
heartrate %>% 
  pivot_longer(-patient,  #patient will not pivot  
               names_to = "drug", #make a new column name
...
This data is not tidy because the column names are data. 
...
Column days are data that need to have their own rows
